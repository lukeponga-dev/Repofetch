...
async function main() {
  ...
  const aiRepos = data.repositories.filter((repo) =>
...
function generateAIAnalytics(repositories) {
  ...
  // Popularity analysis
  const starred = repositories.filter(
  ...
  return {
    ...
    languages: {
      ...
      most_popular: Object.keys(languageCounts).sort(
...
function displayAIAnalytics(analytics) {
  ...
  console.log(
    `Recently Active (30 days): ${summary.recently_active_30_days} (${summary.activity_percentage}%)`
  );
