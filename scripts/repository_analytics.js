...
class RepositoryAnalytics {
  ...
  getSummary() {
    ...
    const aiProjects = this.repositories.filter(
      (repo) =>
        repo.name.toLowerCase().includes("ai") ||
        (repo.description && repo.description.toLowerCase().includes("ai"))
    ).length;

    const totalStars = this.repositories.reduce(
      (sum, repo) => sum + repo.stargazers_count,
      0
    );
    const totalForks = this.repositories.reduce(
      (sum, repo) => sum + repo.forks_count,
      0
    );
    const totalIssues = this.repositories.reduce(
    ...
    return {
      distribution: languages.map((lang) => ({
        ...
        percentage: Math.round(
          (languageCounts[lang] / this.repositories.length) * 100
        ),
      })),
      most_popular_language: languages[0],
      most_starred_language: Object.keys(languageStars).sort(
        (a, b) => languageStars[b] - languageStars[a]
      )[0],
    };
  }
  ...
  getPopularityStats() {
    const repos = [...this.repositories].sort(
      (a, b) => b.stargazers_count - a.stargazers_count
    );
    const forks = [...this.repositories].sort(
  ...
  getAgeAnalysis() {
    ...
    const averageAge = Math.floor(
      ages.reduce((sum, repo) => sum + repo.age_days, 0) / ages.length
    );
    const recentlyUpdated = ages.filter(
      (repo) => repo.days_since_update <= 30
    ).length;
    const recentlyPushed = ages.filter(
      (repo) => repo.days_since_push <= 7
    ).length;
    ...
    return {
      ...
      most_recently_updated: ages
        .sort((a, b) => a.days_since_update - b.days_since_update)
        .slice(0, 5),
  ...
  getActivityStats() {
    ...
    return {
      ...
      activity_percentage: Math.round(
        (activeRepos.length / this.repositories.length) * 100
      ),
  ...
  getPrivacyStats() {
    ...
    return {
      ...
      public_percentage: Math.round(
        (publicRepos.length / this.repositories.length) * 100
      ),
      private_percentage: Math.round(
        (privateRepos.length / this.repositories.length) * 100
      ),
  ...
  // Helper methods
  getMostStarredRepo() {
    const repo = [...this.repositories].sort(
      (a, b) => b.stargazers_count - a.stargazers_count
    )[0];
    return {
      name: repo.name,
...
async function main() {
  try {
    ...
    if (outputJson) {
      ...
      console.log(
        `Total Stars: ${report.summary.total_stars.toLocaleString()}`
      );
      console.log(
        `Total Forks: ${report.summary.total_forks.toLocaleString()}`
      );
      console.log(
        `Average Stars per Repo: ${report.summary.average_stars_per_repo}`
      );
      ...
      report.languages.distribution.slice(0, 5).forEach((lang) => {
        console.log(
          `${lang.language}: ${lang.repository_count} repos (${lang.percentage}%) - ${lang.total_stars} ⭐`
        );
      });
      ...
      console.log(
        `Recently Updated (30 days): ${report.age.recently_updated_30_days}`
      );
      console.log(
        `Recently Active (7 days): ${report.age.recently_pushed_7_days}`
      );

      // Privacy
      console.log("\n🔐 PRIVACY DISTRIBUTION");
      console.log(
        `Public: ${report.privacy.public_repositories} (${report.privacy.public_percentage}%)`
      );
      console.log(
        `Private: ${report.privacy.private_repositories} (${report.privacy.private_percentage}%)`
      );
